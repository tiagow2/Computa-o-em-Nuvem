...
A página web foi desenvolvida com arquitetura visual customizada e exclusiva (*Cloud Workspace Dashboard*), utilizando **Bento Grid**, **Glassmorphism**, paleta em modo escuro com iluminação ambiente difusa (*ambient mesh glow*), tipografia técnica (*Plus Jakarta Sans* e *JetBrains Mono*) e abas interativas para navegação em tempo real pelos 5 Pilares do NIST, Matriz de Responsabilidade Compartilhada e Estudo de Caso Hospitalar.

O código-fonte integral encontra-se versionado na raiz deste repositório no arquivo [**`index.html`**](index.html).
